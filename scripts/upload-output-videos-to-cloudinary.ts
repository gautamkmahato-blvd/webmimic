/**
 * Upload all videos from public/output/ to the Cloudinary "videos" folder.
 *
 * Usage:
 *   npx tsx scripts/upload-output-videos-to-cloudinary.ts
 *   npx tsx scripts/upload-output-videos-to-cloudinary.ts --dry-run
 *   npx tsx scripts/upload-output-videos-to-cloudinary.ts --force
 *
 ...
 *
 * Writes a manifest:
 *   data/motion/cloudinary-videos.json  { "animation_1": "https://res.cloudinary.com/...", ... }
 */
...
const INPUT_DIR = path.join(process.cwd(), 'public', 'output');
const MANIFEST_PATH = path.join(process.cwd(), 'data', 'motion', 'cloudinary-videos.json');
const CLOUDINARY_FOLDER = 'videos';
...
async function main(): Promise<void> {
  ...
  console.log(
    `Uploading ${files.length} video(s) from public/output → Cloudinary folder "${CLOUDINARY_FOLDER}/"${dryRun ? ' [dry-run]' : ''}${force ? ' [force overwrite]' : ''}`,
  );
  ...
  for (const filename of files) {
    ...
    try {
      const result = await uploadVideo(filePath, publicId, force);
