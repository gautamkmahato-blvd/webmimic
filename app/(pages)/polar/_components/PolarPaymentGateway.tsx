...
// ── Plan definitions — polarKey maps to BillingPlanKey for checkout ──
const plans = {
  monthly: [
    {
      ...
      features: freeFeatures, polarKey: null as BillingPlanKey | null,
    },
    {
      ...
      features: basicFeatures, polarKey: 'BASIC' as BillingPlanKey,
    },
    {
      ...
      features: proFeatures, polarKey: 'PRO' as BillingPlanKey,
    },
  ],
  yearly: [
    {
    ...
    },
    {
      ...
      description: 'For designers and developers who extract and recreate components regularly.',
      cta: 'Start with Basic', dark: false, badge: null,
      features: basicFeatures, polarKey: 'BASIC_ANNUALLY' as BillingPlanKey,
    },
    {
      name: 'Pro',   price: 200, period: '/yr', originalPrice: 240, saving: 40,
      description: 'Unlimited power for professionals shipping production-ready work.',
      cta: 'Start with Pro', dark: true, badge: 'Most popular',
      features: proFeatures, polarKey: 'PRO_ANNUALLY' as BillingPlanKey,
    },
  ],
};
...
export default function PolarPaymentGateway() {
  ...
  const createCheckout = async (planKey: BillingPlanKey) => {
    setLoadingKey(planKey);
    setError(null);

    try {
      const productId = getPolarProductId(planKey);
  ...
  return (
    <>
      ...
      <section className="mx-auto w-[min(1160px,calc(100%-48px))] pb-[96px]">
          ...
          {currentPlans.map((plan, i) => (
            ...
            >
              ...
              {plan.polarKey ? (
                <button
                  type="button"
                  onClick={() => createCheckout(plan.polarKey!)}
                  disabled={loadingKey !== null}
                  className={`w-full h-[44px] rounded-[10px] flex items-center justify-center text-[13px] font-bold mb-[24px] transition-opacity hover:opacity-80 disabled:opacity-60 disabled:cursor-not-allowed ${
                    plan.dark
                      ? 'bg-white text-[#080d14]'
                      : 'bg-[#090b12] text-white shadow-[0_4px_14px_rgba(9,11,18,.22)]'
                  }`}
                >
                  {loadingKey === plan.polarKey ? (
                    <span className="flex items-center gap-[8px]">
                      <span className="w-[14px] h-[14px] border-2 border-current border-t-transparent rounded-full animate-spin" />
                      Processing…
                    </span>
                  ) : (
                    plan.cta
                ...
                <a
                  href="#"
                  className="w-full h-[44px] rounded-[10px] flex items-center justify-center text-[13px] font-bold mb-[24px] transition-opacity hover:opacity-80 bg-[#f3f5f8] text-[#374151] border border-[#e5eaf3]"
                >
                  {plan.cta}
                </a>
              )}
        ...
        {/* Error banner */}
        {error && (
          <div className="mt-[24px] max-w-[480px] mx-auto bg-red-50 border border-red-100 rounded-[12px] px-[16px] py-[12px] flex items-start gap-[10px]">
            <AlertCircle size={15} className="text-red-500 shrink-0 mt-[1px]" />
            <div>
              <p className="text-[13px] font-semibold text-red-800">Checkout error</p>
